...
namespace SuperMarkoGUI {
	...
	public:
			...
			array<ADMIN^>^ admins = gcnew array<ADMIN^>(3);
			array<BILL^>^ allBills = gcnew array<BILL^>(MAX_BILLS);
		...
		void loadBillsFromFile() {
			billCount = 0;
			StreamReader^ reader = gcnew StreamReader("view.txt");
			String^ line;

			while ((line = reader->ReadLine()) != nullptr && billCount < MAX_BILLS) { 
				...
				BILL^ bill = gcnew BILL();

								bill->invoiceNumber = parts[0];
							bill->dateTime = parts[1]->Trim();
				
				bill->customerID = parts[2]->Trim();

				String^ productsData = parts[3];
				bill->total = Convert::ToDouble(parts[4]);

				
				array<String^>^ productEntries = productsData->Split(';');
				array<PRODUCTBILL^>^ productArray = gcnew array<PRODUCTBILL^>(productEntries->Length);

				for (int i = 0; i < productEntries->Length; i++) {
					array<String^>^ details = productEntries[i]->Split(',');

					if (details->Length < 4) {
						continue;   
					}

					PRODUCTBILL^ prod = gcnew PRODUCTBILL();
					prod->name = details[0];
					prod->quantity = Convert::ToDouble(details[1]);
					prod->unitPrice = Convert::ToDouble(details[2]);
					prod->subtotal = Convert::ToDouble(details[3]);

					productArray[i] = prod;
				}

				bill->products = productArray;

				
				allBills[billCount++] = bill;
		...
		void displayBills() {
			...
			labelTitle->Text = "All Bills";
			labelTitle->Font = gcnew System::Drawing::Font(L"Segoe UI", 16.0F, FontStyle::Bold);
			labelTitle->AutoSize = true;
			labelTitle->Location = System::Drawing::Point(500, 10);
			labelTitle->ForeColor = Color::FromArgb(230, 52, 98);
			labelTitle->BackColor = Color::Transparent;
			labelTitle->Width = 800; // Set a width to prevent wrapping
			labelTitle->Height = 800; // Set a height to prevent wrapping

			Panel^ headerPanel = gcnew Panel();
			headerPanel->Size = System::Drawing::Size(1100, 50);
			headerPanel->Controls->Add(labelTitle);
			flowLayoutPanel14->Controls->Add(headerPanel);

			for (int i = 0; i < billCount; i++) {
				if (allBills[i] == nullptr) continue;  
				BILL^ bill = allBills[i];

			
				Panel^ billPanel = gcnew Panel();
				billPanel->Size = System::Drawing::Size(500, 300);
				billPanel->BorderStyle = System::Windows::Forms::BorderStyle::FixedSingle;

				// Label: Invoice Number
				Label^ labelInvoiceNumber = gcnew Label();
				labelInvoiceNumber->Font = gcnew System::Drawing::Font(L"Segoe UI", 12.0F, FontStyle::Bold);
				labelInvoiceNumber->AutoSize = false;
				labelInvoiceNumber->Width = 300; // Set a width to prevent wrapping
				labelInvoiceNumber->Height = 20; // Set a height to prevent wrapping
				labelInvoiceNumber->Text = "Invoice Number: " + bill->invoiceNumber;
				labelInvoiceNumber->Location = System::Drawing::Point(10, 10);
				billPanel->Controls->Add(labelInvoiceNumber);

				// Label: Date
				Label^ labelDate = gcnew Label();
				labelDate->Font = gcnew System::Drawing::Font(L"Segoe UI", 12.0F, FontStyle::Bold);
				labelDate->AutoSize = false;
				labelDate->Width = 300; // Set a width to prevent wrapping
				labelDate->Height = 20; // Set a height to prevent wrapping
				labelDate->Text = "Date: " + bill->dateTime;
				labelDate->Location = System::Drawing::Point(10, 30);
				billPanel->Controls->Add(labelDate);

				// Label: Customer ID
				Label^ labelCustomerID = gcnew Label();
				labelCustomerID->Font = gcnew System::Drawing::Font(L"Segoe UI", 12.0F, FontStyle::Bold);
				labelCustomerID->AutoSize = false;
				labelCustomerID->Width = 300; // Set a width to prevent wrapping
				labelCustomerID->Height = 20; // Set a height to prevent wrapping
				labelCustomerID->Text = "Customer ID: " + bill->customerID;
				labelCustomerID->Location = System::Drawing::Point(10, 50);
				billPanel->Controls->Add(labelCustomerID);

				// ListBox: Products
				ListBox^ productListBox = gcnew ListBox();
				productListBox->Font = gcnew System::Drawing::Font(L"Segoe UI", 8.0F, FontStyle::Bold);
				productListBox->Location = System::Drawing::Point(10, 80);
				productListBox->Size = System::Drawing::Size(480, 150);

				if (bill->products != nullptr) {
					for (int j = 0; j < bill->products->Length; j++) {
						if (bill->products[j] == nullptr) continue;

						PRODUCTBILL^ product = bill->products[j];
						productListBox->Items->Add("Product: "+product->name + " , " + "Quantity: " + product->quantity.ToString("F2") + " , " +"Unitprice: " + product->unitPrice.ToString("F2") + " , " +"Subtotal: " + product->subtotal.ToString("F2"));
					}
				}

				billPanel->Controls->Add(productListBox);

				// Label: Total Price
				Label^ labelTotalPrice = gcnew Label();
				labelTotalPrice->Font = gcnew System::Drawing::Font(L"Segoe UI", 12.0F, FontStyle::Bold);
				labelTotalPrice->AutoSize = false;
				labelTotalPrice->Width = 300; // Set a width to prevent wrapping
				labelTotalPrice->Height = 20;
				labelTotalPrice->Text = "Total Price: " + bill->total.ToString("F2");
				labelTotalPrice->Location = System::Drawing::Point(200, 240);
				billPanel->Controls->Add(labelTotalPrice);

				//FlowLayoutPanel
				flowLayoutPanel14->Controls->Add(billPanel);
			}
		}
	...
	void saveGridToFileFormatted() {
	 try {
		   double total = 0.0;
			StreamWriter^ writer = gcnew StreamWriter("view.txt", true);

			writer->Write(tb_invoicenumber_theinvoice->Text + "|");
			writer->Write(tb_date_theinvoice->Text + "|");
			writer->Write(tb_customername_theincoive->Text + "|");

			System::Collections::Generic::List<String^>^ productsList = gcnew System::Collections::Generic::List<String^>();

			for (int row = 0; row < dataGridView1->Rows->Count; row++) {
				if (!dataGridView1->Rows[row]->IsNewRow) {
					System::Text::StringBuilder^ productLine = gcnew System::Text::StringBuilder();

					for (int col = 0; col < dataGridView1->Columns->Count; col++) {
						System::Object^ cellValue = dataGridView1->Rows[row]->Cells[col]->Value;
						System::String^ value = cellValue != nullptr ? cellValue->ToString() : "";

						if (col == 3) {
							total += Convert::ToDouble(cellValue);
						}

						if (col == 0 || col == 1 || col == 2) {
							productLine->Append(value + ",");
						}
					else {
						productLine->Append(value);
						}
					...
					productsList->Add(productLine->ToString());
				}
			}
			writer->Write(String::Join(";", productsList));

			writer->Write("|");
			writer->Write(total.ToString("F2"));
			writer->WriteLine();
...
private: System::Windows::Forms::Panel^ pn_after_start;
private: System::Windows::Forms::Button^ btn_user;
private: System::Windows::Forms::Panel^ pn_login_admin;
private: System::Windows::Forms::PictureBox^ pictureBox17;
private: System::Windows::Forms::PictureBox^ pictureBox18;
private: System::Windows::Forms::PictureBox^ pictureBox19;
private: System::Windows::Forms::Label^ label35;
private: System::Windows::Forms::Label^ lb_message_password_admin;

private: System::Windows::Forms::Label^ lb_message_name_admin;

private: System::Windows::Forms::Button^ btn_refresh_password_admin;

private: System::Windows::Forms::Button^ btn_refresh_name_admin;
private: System::Windows::Forms::Button^ btn_seepasword_admin;
...
private: System::Windows::Forms::Label^ lb_password_admin;

private: System::Windows::Forms::Label^ lb_name_admin;
private: System::Windows::Forms::Button^ btn_back_admin;


private: System::Windows::Forms::Button^ btn_login_admin;

private: System::Windows::Forms::Panel^ pn_admin_invoice;
private: System::Windows::Forms::Button^ btn_admin;
private: System::Windows::Forms::Button^ btn_back_pn_after_start;
...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->pn_admin_invoice = (gcnew System::Windows::Forms::Panel());
			this->flowLayoutPanel14 = (gcnew System::Windows::Forms::FlowLayoutPanel());
			this->pn_login_admin = (gcnew System::Windows::Forms::Panel());
			this->pictureBox17 = (gcnew System::Windows::Forms::PictureBox());
			this->pictureBox18 = (gcnew System::Windows::Forms::PictureBox());
			this->pictureBox19 = (gcnew System::Windows::Forms::PictureBox());
			this->label35 = (gcnew System::Windows::Forms::Label());
			this->lb_message_password_admin = (gcnew System::Windows::Forms::Label());
			this->lb_message_name_admin = (gcnew System::Windows::Forms::Label());
			this->btn_refresh_password_admin = (gcnew System::Windows::Forms::Button());
			this->btn_refresh_name_admin = (gcnew System::Windows::Forms::Button());
			this->btn_seepasword_admin = (gcnew System::Windows::Forms::Button());
			this->tb_password_admin = (gcnew System::Windows::Forms::TextBox());
			this->tb_name_admin = (gcnew System::Windows::Forms::TextBox());
			this->lb_password_admin = (gcnew System::Windows::Forms::Label());
			this->lb_name_admin = (gcnew System::Windows::Forms::Label());
			this->btn_back_admin = (gcnew System::Windows::Forms::Button());
			this->btn_login_admin = (gcnew System::Windows::Forms::Button());
			this->pn_after_start = (gcnew System::Windows::Forms::Panel());
			this->btn_back_pn_after_start = (gcnew System::Windows::Forms::Button());
			...
			this->pn_admin_invoice->SuspendLayout();
			this->pn_login_admin->SuspendLayout();
			(cli::safe_cast<System::ComponentModel::ISupportInitialize^>(this->pictureBox17))->BeginInit();
			(cli::safe_cast<System::ComponentModel::ISupportInitialize^>(this->pictureBox18))->BeginInit();
			(cli::safe_cast<System::ComponentModel::ISupportInitialize^>(this->pictureBox19))->BeginInit();
			this->pn_after_start->SuspendLayout();
			...
			this->pn_main_dashboard->Controls->Add(this->pn_admin_invoice);
			this->pn_main_dashboard->Controls->Add(this->pn_login_admin);
			this->pn_main_dashboard->Controls->Add(this->pn_after_start);
			...
			// pn_admin_invoice
			// 
			this->pn_admin_invoice->BackColor = System::Drawing::Color::FromArgb(static_cast<System::Int32>(static_cast<System::Byte>(239)),
				static_cast<System::Int32>(static_cast<System::Byte>(239)), static_cast<System::Int32>(static_cast<System::Byte>(230)));
			this->pn_admin_invoice->BackgroundImageLayout = System::Windows::Forms::ImageLayout::Stretch;
			this->pn_admin_invoice->Controls->Add(this->flowLayoutPanel14);
			this->pn_admin_invoice->Dock = System::Windows::Forms::DockStyle::Fill;
			this->pn_admin_invoice->Location = System::Drawing::Point(0, 0);
			this->pn_admin_invoice->Margin = System::Windows::Forms::Padding(3, 2, 3, 2);
			this->pn_admin_invoice->Name = L"pn_admin_invoice";
			this->pn_admin_invoice->Size = System::Drawing::Size(1485, 745);
			this->pn_admin_invoice->TabIndex = 7;
			// 
			// flowLayoutPanel14
			// 
			this->flowLayoutPanel14->AutoScroll = true;
			this->flowLayoutPanel14->Dock = System::Windows::Forms::DockStyle::Fill;
			this->flowLayoutPanel14->Location = System::Drawing::Point(0, 0);
			this->flowLayoutPanel14->Name = L"flowLayoutPanel14";
			this->flowLayoutPanel14->Size = System::Drawing::Size(1485, 745);
			this->flowLayoutPanel14->TabIndex = 0;
			// 
			// pn_login_admin
			// 
			this->pn_login_admin->BackColor = System::Drawing::Color::FromArgb(static_cast<System::Int32>(static_cast<System::Byte>(51)), static_cast<System::Int32>(static_cast<System::Byte>(55)),
				static_cast<System::Int32>(static_cast<System::Byte>(69)));
			this->pn_login_admin->BackgroundImageLayout = System::Windows::Forms::ImageLayout::Stretch;
			this->pn_login_admin->Controls->Add(this->pictureBox17);
			this->pn_login_admin->Controls->Add(this->pictureBox18);
			this->pn_login_admin->Controls->Add(this->pictureBox19);
			this->pn_login_admin->Controls->Add(this->label35);
			this->pn_login_admin->Controls->Add(this->lb_message_password_admin);
			this->pn_login_admin->Controls->Add(this->lb_message_name_admin);
			this->pn_login_admin->Controls->Add(this->btn_refresh_password_admin);
			this->pn_login_admin->Controls->Add(this->btn_refresh_name_admin);
			this->pn_login_admin->Controls->Add(this->btn_seepasword_admin);
			this->pn_login_admin->Controls->Add(this->tb_password_admin);
			this->pn_login_admin->Controls->Add(this->tb_name_admin);
			this->pn_login_admin->Controls->Add(this->lb_password_admin);
			this->pn_login_admin->Controls->Add(this->lb_name_admin);
			this->pn_login_admin->Controls->Add(this->btn_back_admin);
			this->pn_login_admin->Controls->Add(this->btn_login_admin);
			this->pn_login_admin->Dock = System::Windows::Forms::DockStyle::Fill;
			this->pn_login_admin->Location = System::Drawing::Point(0, 0);
			this->pn_login_admin->Margin = System::Windows::Forms::Padding(3, 2, 3, 2);
			this->pn_login_admin->Name = L"pn_login_admin";
			this->pn_login_admin->Size = System::Drawing::Size(1485, 745);
			this->pn_login_admin->TabIndex = 6;
			// 
			// pictureBox17
			// 
			this->pictureBox17->Image = (cli::safe_cast<System::Drawing::Image^>(resources->GetObject(L"pictureBox17.Image")));
			this->pictureBox17->Location = System::Drawing::Point(-39, 507);
			this->pictureBox17->Margin = System::Windows::Forms::Padding(4);
			this->pictureBox17->Name = L"pictureBox17";
			this->pictureBox17->Size = System::Drawing::Size(432, 343);
			...
			this->pictureBox18->Location = System::Drawing::Point(1093, 507);
			this->pictureBox18->Margin = System::Windows::Forms::Padding(4);
			this->pictureBox18->Name = L"pictureBox18";
			this->pictureBox18->Size = System::Drawing::Size(432, 343);
			...
			this->pictureBox19->Location = System::Drawing::Point(588, 54);
			this->pictureBox19->Margin = System::Windows::Forms::Padding(4);
			this->pictureBox19->Name = L"pictureBox19";
			this->pictureBox19->Size = System::Drawing::Size(87, 62);
			...
			this->label35->Font = (gcnew System::Drawing::Font(L"Segoe UI", 27.75F, System::Drawing::FontStyle::Bold, System::Drawing::GraphicsUnit::Point,
				static_cast<System::Byte>(0)));
			this->label35->ForeColor = System::Drawing::Color::White;
			this->label35->Location = System::Drawing::Point(684, 48);
			this->label35->Margin = System::Windows::Forms::Padding(4, 0, 4, 0);
			this->label35->Name = L"label35";
			this->label35->Size = System::Drawing::Size(213, 68);
			this->label35->TabIndex = 22;
			this->label35->Text = L"Login";
			// 
			// lb_message_password_admin
			// 
			this->lb_message_password_admin->BackColor = System::Drawing::Color::Transparent;
			this->lb_message_password_admin->Font = (gcnew System::Drawing::Font(L"Microsoft Sans Serif", 10.2F, System::Drawing::FontStyle::Regular,
				System::Drawing::GraphicsUnit::Point, static_cast<System::Byte>(0)));
			this->lb_message_password_admin->ForeColor = System::Drawing::Color::FromArgb(static_cast<System::Int32>(static_cast<System::Byte>(254)),
				static_cast<System::Int32>(static_cast<System::Byte>(95)), static_cast<System::Int32>(static_cast<System::Byte>(85)));
			this->lb_message_password_admin->Location = System::Drawing::Point(653, 362);
			this->lb_message_password_admin->Name = L"lb_message_password_admin";
			this->lb_message_password_admin->Size = System::Drawing::Size(325, 23);
			this->lb_message_password_admin->TabIndex = 21;
			this->lb_message_password_admin->Text = L"Please enter the password";
			this->lb_message_password_admin->Visible = false;
			// 
			// lb_message_name_admin
			// 
			this->lb_message_name_admin->BackColor = System::Drawing::Color::Transparent;
			this->lb_message_name_admin->Font = (gcnew System::Drawing::Font(L"Microsoft Sans Serif", 10.2F, System::Drawing::FontStyle::Regular,
				System::Drawing::GraphicsUnit::Point, static_cast<System::Byte>(0)));
			this->lb_message_name_admin->ForeColor = System::Drawing::Color::FromArgb(static_cast<System::Int32>(static_cast<System::Byte>(254)),
				static_cast<System::Int32>(static_cast<System::Byte>(95)), static_cast<System::Int32>(static_cast<System::Byte>(85)));
			this->lb_message_name_admin->Location = System::Drawing::Point(648, 252);
			this->lb_message_name_admin->Name = L"lb_message_name_admin";
			this->lb_message_name_admin->Size = System::Drawing::Size(325, 23);
			this->lb_message_name_admin->TabIndex = 20;
			this->lb_message_name_admin->Text = L" Please enter the username";
			this->lb_message_name_admin->Visible = false;
			// 
			// btn_refresh_password_admin
			// 
			this->btn_refresh_password_admin->BackColor = System::Drawing::Color::FromArgb(static_cast<System::Int32>(static_cast<System::Byte>(239)),
				static_cast<System::Int32>(static_cast<System::Byte>(239)), static_cast<System::Int32>(static_cast<System::Byte>(239)));
			this->btn_refresh_password_admin->BackgroundImage = (cli::safe_cast<System::Drawing::Image^>(resources->GetObject(L"btn_refresh_password_admin.BackgroundImage")));
			this->btn_refresh_password_admin->BackgroundImageLayout = System::Windows::Forms::ImageLayout::Zoom;
			this->btn_refresh_password_admin->FlatAppearance->BorderSize = 0;
			this->btn_refresh_password_admin->FlatStyle = System::Windows::Forms::FlatStyle::Flat;
			this->btn_refresh_password_admin->Location = System::Drawing::Point(995, 331);
			this->btn_refresh_password_admin->Margin = System::Windows::Forms::Padding(3, 2, 3, 2);
			this->btn_refresh_password_admin->Name = L"btn_refresh_password_admin";
			this->btn_refresh_password_admin->Size = System::Drawing::Size(39, 23);
			this->btn_refresh_password_admin->TabIndex = 19;
			this->btn_refresh_password_admin->UseVisualStyleBackColor = false;
			this->btn_refresh_password_admin->Click += gcnew System::EventHandler(this, &MyForm::btn_refresh_password_admin_Click);
			// 
			// btn_refresh_name_admin
			// 
			this->btn_refresh_name_admin->BackColor = System::Drawing::Color::FromArgb(static_cast<System::Int32>(static_cast<System::Byte>(239)),
				static_cast<System::Int32>(static_cast<System::Byte>(239)), static_cast<System::Int32>(static_cast<System::Byte>(239)));
			this->btn_refresh_name_admin->BackgroundImage = (cli::safe_cast<System::Drawing::Image^>(resources->GetObject(L"btn_refresh_name_admin.BackgroundImage")));
			this->btn_refresh_name_admin->BackgroundImageLayout = System::Windows::Forms::ImageLayout::Zoom;
			this->btn_refresh_name_admin->FlatAppearance->BorderSize = 0;
			this->btn_refresh_name_admin->FlatStyle = System::Windows::Forms::FlatStyle::Flat;
			this->btn_refresh_name_admin->ForeColor = System::Drawing::Color::White;
			this->btn_refresh_name_admin->Location = System::Drawing::Point(995, 217);
			this->btn_refresh_name_admin->Margin = System::Windows::Forms::Padding(3, 2, 3, 2);
			this->btn_refresh_name_admin->Name = L"btn_refresh_name_admin";
			this->btn_refresh_name_admin->Size = System::Drawing::Size(39, 23);
			this->btn_refresh_name_admin->TabIndex = 18;
			this->btn_refresh_name_admin->UseVisualStyleBackColor = false;
			this->btn_refresh_name_admin->Click += gcnew System::EventHandler(this, &MyForm::btn_refresh_name_admin_Click);
			// 
			// btn_seepasword_admin
			// 
			this->btn_seepasword_admin->BackColor = System::Drawing::Color::White;
			this->btn_seepasword_admin->BackgroundImage = (cli::safe_cast<System::Drawing::Image^>(resources->GetObject(L"btn_seepasword_admin.BackgroundImage")));
			this->btn_seepasword_admin->BackgroundImageLayout = System::Windows::Forms::ImageLayout::Zoom;
			this->btn_seepasword_admin->FlatAppearance->BorderSize = 0;
			this->btn_seepasword_admin->FlatStyle = System::Windows::Forms::FlatStyle::Flat;
			this->btn_seepasword_admin->Location = System::Drawing::Point(940, 324);
			this->btn_seepasword_admin->Margin = System::Windows::Forms::Padding(3, 2, 3, 2);
			this->btn_seepasword_admin->Name = L"btn_seepasword_admin";
			this->btn_seepasword_admin->Size = System::Drawing::Size(33, 34);
			this->btn_seepasword_admin->TabIndex = 15;
			this->btn_seepasword_admin->UseVisualStyleBackColor = false;
			this->btn_seepasword_admin->Click += gcnew System::EventHandler(this, &MyForm::btn_seepasword_admin_Click);
			// 
			// tb_password_admin
			// 
			this->tb_password_admin->BorderStyle = System::Windows::Forms::BorderStyle::FixedSingle;
			this->tb_password_admin->Font = (gcnew System::Drawing::Font(L"Microsoft Sans Serif", 15.75F, System::Drawing::FontStyle::Regular,
				System::Drawing::GraphicsUnit::Point, static_cast<System::Byte>(0)));
			this->tb_password_admin->Location = System::Drawing::Point(645, 322);
			this->tb_password_admin->Margin = System::Windows::Forms::Padding(3, 2, 3, 2);
			this->tb_password_admin->Name = L"tb_password_admin";
			this->tb_password_admin->Size = System::Drawing::Size(330, 37);
			this->tb_password_admin->TabIndex = 1;
			this->tb_password_admin->UseSystemPasswordChar = true;
			// 
			// tb_name_admin
			// 
			this->tb_name_admin->BorderStyle = System::Windows::Forms::BorderStyle::FixedSingle;
			this->tb_name_admin->Font = (gcnew System::Drawing::Font(L"Microsoft Sans Serif", 15.75F, System::Drawing::FontStyle::Regular, System::Drawing::GraphicsUnit::Point,
				static_cast<System::Byte>(0)));
			this->tb_name_admin->Location = System::Drawing::Point(645, 210);
			this->tb_name_admin->Margin = System::Windows::Forms::Padding(3, 2, 3, 2);
			this->tb_name_admin->Name = L"tb_name_admin";
			this->tb_name_admin->Size = System::Drawing::Size(330, 37);
			this->tb_name_admin->TabIndex = 0;
			// 
			// lb_password_admin
			// 
			this->lb_password_admin->BackColor = System::Drawing::Color::Transparent;
			this->lb_password_admin->Font = (gcnew System::Drawing::Font(L"Segoe UI", 18, System::Drawing::FontStyle::Bold, System::Drawing::GraphicsUnit::Point,
				static_cast<System::Byte>(0)));
			this->lb_password_admin->ForeColor = System::Drawing::Color::White;
			this->lb_password_admin->Location = System::Drawing::Point(452, 319);
			this->lb_password_admin->Name = L"lb_password_admin";
			this->lb_password_admin->Size = System::Drawing::Size(177, 34);
			this->lb_password_admin->TabIndex = 4;
			this->lb_password_admin->Text = L"Password";
			// 
			// lb_name_admin
			// 
			this->lb_name_admin->BackColor = System::Drawing::Color::Transparent;
			this->lb_name_admin->Font = (gcnew System::Drawing::Font(L"Segoe UI", 18, System::Drawing::FontStyle::Bold, System::Drawing::GraphicsUnit::Point,
				static_cast<System::Byte>(0)));
			this->lb_name_admin->ForeColor = System::Drawing::Color::White;
			this->lb_name_admin->Location = System::Drawing::Point(452, 206);
			this->lb_name_admin->Name = L"lb_name_admin";
			this->lb_name_admin->Size = System::Drawing::Size(177, 34);
			this->lb_name_admin->TabIndex = 3;
			this->lb_name_admin->Text = L"Username";
			// 
			// btn_back_admin
			// 
			this->btn_back_admin->BackColor = System::Drawing::Color::DarkGray;
			this->btn_back_admin->BackgroundImageLayout = System::Windows::Forms::ImageLayout::Center;
			this->btn_back_admin->FlatAppearance->BorderSize = 0;
			this->btn_back_admin->FlatStyle = System::Windows::Forms::FlatStyle::Flat;
			this->btn_back_admin->Font = (gcnew System::Drawing::Font(L"Microsoft Sans Serif", 16.2F, System::Drawing::FontStyle::Regular, System::Drawing::GraphicsUnit::Point,
				static_cast<System::Byte>(0)));
			this->btn_back_admin->ForeColor = System::Drawing::Color::White;
			this->btn_back_admin->Location = System::Drawing::Point(664, 607);
			this->btn_back_admin->Margin = System::Windows::Forms::Padding(3, 2, 3, 2);
			this->btn_back_admin->Name = L"btn_back_admin";
			this->btn_back_admin->Size = System::Drawing::Size(152, 53);
			this->btn_back_admin->TabIndex = 4;
			this->btn_back_admin->Text = L"Back";
			this->btn_back_admin->UseVisualStyleBackColor = false;
			this->btn_back_admin->Click += gcnew System::EventHandler(this, &MyForm::btn_back_admin_Click);
			// 
			// btn_login_admin
			// 
			this->btn_login_admin->BackColor = System::Drawing::Color::Turquoise;
			this->btn_login_admin->BackgroundImageLayout = System::Windows::Forms::ImageLayout::Center;
			this->btn_login_admin->FlatAppearance->BorderSize = 0;
			this->btn_login_admin->FlatStyle = System::Windows::Forms::FlatStyle::Flat;
			this->btn_login_admin->Font = (gcnew System::Drawing::Font(L"Microsoft Sans Serif", 17.25F, System::Drawing::FontStyle::Bold, System::Drawing::GraphicsUnit::Point,
				static_cast<System::Byte>(0)));
			this->btn_login_admin->ForeColor = System::Drawing::Color::White;
			this->btn_login_admin->Location = System::Drawing::Point(625, 519);
			this->btn_login_admin->Margin = System::Windows::Forms::Padding(3, 2, 3, 2);
			this->btn_login_admin->Name = L"btn_login_admin";
			this->btn_login_admin->Size = System::Drawing::Size(229, 58);
			this->btn_login_admin->TabIndex = 2;
			this->btn_login_admin->Text = L"Log in";
			this->btn_login_admin->UseVisualStyleBackColor = false;
			this->btn_login_admin->Click += gcnew System::EventHandler(this, &MyForm::btn_login_admin_Click);
			// 
			// pn_after_start
			// 
			this->pn_after_start->BackColor = System::Drawing::Color::FromArgb(static_cast<System::Int32>(static_cast<System::Byte>(51)), static_cast<System::Int32>(static_cast<System::Byte>(55)),
				static_cast<System::Int32>(static_cast<System::Byte>(69)));
			this->pn_after_start->Controls->Add(this->btn_back_pn_after_start);
			this->pn_after_start->Controls->Add(this->btn_admin);
			this->pn_after_start->Controls->Add(this->btn_user);
			this->pn_after_start->Dock = System::Windows::Forms::DockStyle::Fill;
			this->pn_after_start->Location = System::Drawing::Point(0, 0);
			this->pn_after_start->Margin = System::Windows::Forms::Padding(3, 2, 3, 2);
			this->pn_after_start->Name = L"pn_after_start";
			this->pn_after_start->Size = System::Drawing::Size(1485, 745);
			this->pn_after_start->TabIndex = 5;
			// 
			// btn_back_pn_after_start
			// 
			this->btn_back_pn_after_start->Font = (gcnew System::Drawing::Font(L"Microsoft Sans Serif", 16.2F, System::Drawing::FontStyle::Bold,
				System::Drawing::GraphicsUnit::Point, static_cast<System::Byte>(0)));
			this->btn_back_pn_after_start->Location = System::Drawing::Point(621, 426);
			this->btn_back_pn_after_start->Name = L"btn_back_pn_after_start";
			this->btn_back_pn_after_start->Size = System::Drawing::Size(234, 71);
			this->btn_back_pn_after_start->TabIndex = 2;
			this->btn_back_pn_after_start->Text = L"Back";
			this->btn_back_pn_after_start->UseVisualStyleBackColor = true;
			this->btn_back_pn_after_start->Click += gcnew System::EventHandler(this, &MyForm::btn_back_pn_after_start_Click);
			// 
			// btn_admin
			// 
			this->btn_admin->Font = (gcnew System::Drawing::Font(L"Microsoft Sans Serif", 16.2F, System::Drawing::FontStyle::Bold, System::Drawing::GraphicsUnit::Point,
				static_cast<System::Byte>(0)));
			this->btn_admin->Location = System::Drawing::Point(725, 202);
			this->btn_admin->Name = L"btn_admin";
			this->btn_admin->Size = System::Drawing::Size(234, 71);
			...
			// btn_user
			// 
			this->btn_user->Font = (gcnew System::Drawing::Font(L"Microsoft Sans Serif", 16.2F, System::Drawing::FontStyle::Bold, System::Drawing::GraphicsUnit::Point,
				static_cast<System::Byte>(0)));
			this->btn_user->Location = System::Drawing::Point(263, 206);
			this->btn_user->Name = L"btn_user";
			this->btn_user->Size = System::Drawing::Size(234, 71);
			this->btn_user->TabIndex = 0;
			this->btn_user->Text = L"User";
			...
			// 
			this->btn_refresh_password_login->BackColor = System::Drawing::Color::FromArgb(static_cast<System::Int32>(static_cast<System::Byte>(239)),
				static_cast<System::Int32>(static_cast<System::Byte>(239)), static_cast<System::Int32>(static_cast<System::Byte>(239)));
			...
			// 
			this->btn_refresh_username_login->BackColor = System::Drawing::Color::FromArgb(static_cast<System::Int32>(static_cast<System::Byte>(239)),
				static_cast<System::Int32>(static_cast<System::Byte>(239)), static_cast<System::Int32>(static_cast<System::Byte>(239)));
			...
			// 
			this->btn_refresh_location_register->BackColor = System::Drawing::Color::White;
			...
			// 
			this->btn_refresh_phonenumber_register->BackColor = System::Drawing::Color::White;
			...
			// 
			this->btn_refresh_password_register->BackColor = System::Drawing::Color::White;
			...
			// 
			this->btn_refresh_username_register->BackColor = System::Drawing::Color::White;
			...
			this->label28->Location = System::Drawing::Point(303, 416);
			this->label28->Margin = System::Windows::Forms::Padding(4, 0, 4, 0);
			this->label28->Name = L"label28";
			this->label28->Size = System::Drawing::Size(0, 16);
			...
			this->pn_admin_invoice->ResumeLayout(false);
			this->pn_login_admin->ResumeLayout(false);
			this->pn_login_admin->PerformLayout();
			(cli::safe_cast<System::ComponentModel::ISupportInitialize^>(this->pictureBox17))->EndInit();
			(cli::safe_cast<System::ComponentModel::ISupportInitialize^>(this->pictureBox18))->EndInit();
			(cli::safe_cast<System::ComponentModel::ISupportInitialize^>(this->pictureBox19))->EndInit();
			this->pn_after_start->ResumeLayout(false);
...
private: System::Void btn_admin_Click(System::Object^ sender, System::EventArgs^ e) {
	showPanel(pn_login_admin);
}
private: System::Void btn_refresh_name_admin_Click(System::Object^ sender, System::EventArgs^ e) {
	tb_name_admin->Text = "";
}
private: System::Void btn_refresh_password_admin_Click(System::Object^ sender, System::EventArgs^ e) {
	tb_password_admin->Text = "";
}
private: System::Void btn_seepasword_admin_Click(System::Object^ sender, System::EventArgs^ e) {
	if (tb_password_admin->UseSystemPasswordChar == true)
	{
		tb_password_admin->UseSystemPasswordChar = false;
	}
	else
	{
		tb_password_admin->UseSystemPasswordChar = true;
	}
}
private: System::Void btn_back_admin_Click(System::Object^ sender, System::EventArgs^ e) {
	showPanel(pn_after_start);
}
private: System::Void btn_back_pn_after_start_Click(System::Object^ sender, System::EventArgs^ e) {
	showPanel(pn_start);
}
	   private: System::Void loadfromfile()
	   {
		   try
		   {
			   StreamReader^ reader = gcnew StreamReader("admin.txt");
			   String^ line;
			   int index = 0;

			   while ((line = reader->ReadLine()) != nullptr && index < 5)
			   {
				   array < String^>^ parts = line->Split('|');
				   if (parts->Length >= 3)
				   {
...
private: System::Void btn_login_admin_Click(System::Object^ sender, System::EventArgs^ e) {
	loadfromfile();
	if (tb_name_admin->Text == "" || tb_password_admin->Text == "")
	{
	...
	for (int i = 0; i < 3; i++)
	{
		if (admins[i] != nullptr && admins[i]->Name->Trim() == tb_name_admin->Text->Trim() && admins[i]->Password->Trim() == tb_password_admin->Text->Trim())
		{
			MessageBox::Show("Welcome " + admins[i]->Name);
			showPanel(pn_admin_invoice);
			loadBillsFromFile();
			displayBills();
			return;
		}
